...
Running `load_all()` is the fastest way to re-load the functions in your package as if you had installed it and run `library(pkgName)`. It is *MUCH* faster than `install()` (especially if you have compiled code in the package).
...
But you could also add this to `.Rbuildignore` and/or `.gitignore`.
